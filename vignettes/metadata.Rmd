...
The `outpack` package is the reference implementation at the moment of the outpack specification; a collection of schemas and directory structures that outpack requires. Once we reach version 1.0.0, we will split this specification from the package.
...
Each "packet" was conceptually a directory, corresponding to a particular analysis or data product, though this is not necessarily how it is stored. The internal representation includes:
...
* actual file contents of every included file. This might be stored in a simple content-addressable file storage system (see below) or tual copies of the directories as originally created
...
We will need to distinguish between packets which are "unpacked" (that is, packets with every file available in the current archive) and packets that are merely known about (those for which we have the metadata but not the files).
...
The outpack configuration schema is defined in [`config.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/config.json)

The configuration format is still subject to change...

## Packet metadata (`.outpack/metadata/`)

Each file within this directory has a filename that is an outpack id (matching the regular expression `^[0-9]{8}-[0-9]{6}-[0-9a-f]{8}$`, see below. Each file is a json file conforming to the schema [`metadata.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/metadata.json).
...
This directory matches the regular expression `^[0-9]{8}$` (e.g., `457f4f2a`) and is a "location id" (see below) corresponding to a "location". Each file within this directory has an outpack id as name, and contains json about when that location unpacked (or installed) the packet, and the hash of the metadata. This file conforms to the schema [`location.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/location.json).
...
If the configurtion option `core.path_archive` is non-`null` then there will be a directory with that path containing unpacked packets. Each packet will be available at the path

```
archive/<name>/<id>/<files...>
```

With `<name>` being the "name" of the packet, `<id>` being its outpack id.  There will be several files per packet, possibly themselves in directories. This storage approach is designed to be human readable, and will typically only be enabled where 

# Adding a packet

In order to make a packet available from a location, you need to import the metadata and the files

## Adding metadata

Metadata lives in `.outpack/metadata/<id>` where `<id>` is the packet id and confirms to the [`metadata.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/metadata.json) schema. You can simply copy metadata into this directory, but it is not really made available until it is known at a location.  Dangling metadata (that is, metadata present in this directory but not known anywhere) is currently mostly ignored.

You then need to mark the metadata as "known"; to do this write out a small bit of JSON as `.outpack/location/<id>` following the [`metadata.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/location.json) schema. The time here is the time that the location (i.e., us) first learnt about the packet.
...
conforming to the [`location.json`](https://github.com/mrc-ide/outpack/blob/main/inst/schema/location.json) schema, and containing the packet id, the time that it was marked as unpacked and the hash of the metadata.

## Ordering of operations

We only need both files and metadata once the packet is marked as unpacked; note that some configurations guarantee that every packet is unpacked ina  complete tree.
...
The id tries to balance a reasonable degree of collision resistance (65536 combinations per millisecond), lexographic sortability and a reasonble degree of meaningfulness.
...
The schema curently assumes that the hash value is represented as a hex string.
...
The reason for the second level is to prevent perfomance degredation with directories containing millions of files, again copying git.
