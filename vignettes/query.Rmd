...
If we want to search for the version of `D` which depends on the version of `A` which was used in the latest `C` we can do this via

```
outpack_search(quote(latest(uses(single(usedby(latest(name == "C")))))),
               scope = quote(name == "D"))
```
